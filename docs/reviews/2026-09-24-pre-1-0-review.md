...
|---|---|---|---|
| [H](#h--resolve-styling-once-inside-help) | Resolve styling once, inside help | Strong | Shipped: #117, PR #119, `df0e1d3`; #118, PR #120, `018e829`; ADR 0057 |
| [I](#i--prose-becomes-a-typed-module) | Prose becomes a typed module | Strong | Shipped: #132, PR #134, `d071bed`; #133, PR #135, `efddc1d`; ADR 0060 |
| [J](#j--one-shape-for-every-outcome) | One shape for every outcome | Strong | Shipped: #127, PR #129, `24aa0f7`; #128, PR #130, `ab4b6f8`; ADR 0059 |
| [K](#k--settings-live-at-the-root-terminal-probing-in-one-module) | Settings live at the root; terminal probing in one module | Strong | Shipped: #122, PR #124, `8e71e10`; #123, PR #125, `c293fae`; ADR 0058 |
| [L](#l--a-usage-line-module) | A Usage Line module | Worth exploring | Shipped: #137, PR #140, `1783887`; #138, PR #141, `08fac71`; #139, PR #142, `b0256e7`; ADR 0061 |
...
`markup` runs once, in help. **Shipped differently:** the ticks became
`srTick` spans instead, so `validatorHelp` keeps its roles; see
[ADR 0057](../adr/0057-help-context.md).
...
wrap. `Row.text` becomes that type. **Shipped** as the `Prose` type in
`prose.nim`, with two changes: `markup` stayed in `style.nim`, since it is
inline and has callers that never touch blocks, and Column Style's
continuation indent turned out to be an accident, so it was dropped
(#132) rather than made reachable. `wrap` is `Prose`'s only public
operation.
...
place that quits. **Shipped as proposed**, with the policy in a withheld
`outcome` module that returns it as data for the facade to print, and
`SpecDefect`'s label folded in.
...
as a parameter, as `wantsColor` already does for its tests. **Shipped
differently:** `SpecSettings` detects width and colour on first read
instead, so no Spec, root or nested, probes the terminal until output is
rendered, and `command()` is unchanged; see
[ADR 0058](../adr/0058-lazy-terminal-detection.md).
...
**Shipped** as `usage.nim`, with two changes. The split rule became the
parser's (#137): help's blank lines were bare command lines the parser
never built. And a Bare Call got its own syntax instead, a line of only
`{cmd}` (#139), which may start any Usage Line. `autoFillUsage` stayed in
`specbuild.nim`, since it splices through the parser.
