...
Two alternatives were rejected for this. `RawToken.subIdx` cannot carry it:
it equals "is a peel" (already answered by `fromCluster`), the failing letter
is already the remainder's second character, and it holds no text at all —
it is ranking-only (ADR 0036). Recovering the origin from `RawToken.idx`
